...
async function problemA() {
  ...
  // AsyncAwait version
  const readAsyncFile = async (filename) => {
    blue(await promisifiedReadFile(filename));
  };
  async function problemA_AsyncAwait() {
    await Promise.all([
      readAsyncFile("poem-two/stanza-01.txt"),
      readAsyncFile("poem-two/stanza-02.txt"),
    ]);
...
function problemB() {
  ...
  const promiseArray = filenames.map((element) => readAsyncFile(element));
  async function problemA_AsyncAwait() {
...
function problemC() {
  ...
  async function problemC_AsyncAwait() {
    for (let i = 0; i < filenames.length; i++) {
      await readAsyncFile(filenames[i]);
    }
...
function problemD() {
  ...
  async function problemD_AsyncAwait() {
    try {
      for (let i = 0; i < filenames.length; i++) {
        await readAsyncFile(filenames[i]);
      }
